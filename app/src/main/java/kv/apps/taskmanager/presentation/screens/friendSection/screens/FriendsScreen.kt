...
import kv.apps.taskmanager.presentation.screens.friendSection.friendsComposables.FriendCard
import kv.apps.taskmanager.presentation.screens.friendSection.friendsComposables.PendingFriendRequestCard
...
fun FriendsScreen(
    ...
    startWithRequestsTab: Boolean
) {
    val currentUserId = authViewModel.uiState.collectAsState().value.userId
    val hasUserId = currentUserId != null


    val userFriendsUiState by userFriendsViewModel.uiState.collectAsState()
    val friendsState = userFriendsUiState.friends
    val pendingRequestsState = userFriendsUiState.pendingFriendRequests
    val isLoading = userFriendsUiState.isLoading
    ...
    LaunchedEffect(currentUserId) {
        if (currentUserId != null && !hasLoadedInitialData) {
            ...
            hasLoadedInitialData = startWithRequestsTab
        }
    }
    ...
    LaunchedEffect(navController) {
        ...
        navController.currentBackStackEntry?.savedStateHandle?.get<Boolean>("friendAdded")?.let {
            if (it) {
                showAddSuccessDialog = startWithRequestsTab
                navController.currentBackStackEntry?.savedStateHandle?.remove<Boolean>("friendAdded")
            }
        }
    }
            ...
            Column(
                ...
                Column(
                    ...
                    Column(
                        ...
                        when {
                            userFriendsUiState.isLoadingPendingRequests -> {
                                Box(
                                    modifier = Modifier
                                        .fillMaxWidth()
                                        .height(100.dp),
                            ...
                            pendingRequestsState != null -> {
                                when {
                                    pendingRequestsState.isSuccess -> {
                                        val requests = pendingRequestsState.getOrNull() ?: emptyList()
                                    ...
                                    pendingRequestsState.isFailure -> {
                                        val error = pendingRequestsState.exceptionOrNull()
                                        val errorMessage = error?.message ?: "Failed to load requests"
                                        ...
                                        Column(
                                            ...
                                            Button(
                                                onClick = {
                                                    currentUserId.let {
                                                        userFriendsViewModel.getPendingFriendRequests(it)
                                                    }
                                                },
                        ...
                        when {
                            userFriendsUiState.isLoadingFriends -> {
                                Box(
                                    modifier = Modifier
                                        .fillMaxWidth()
                                        .weight(1f),
                                            ...
                                            FriendCard(
                                                ...
                                                onClick = {
                                                    navController.navigate(
                                                        Screen.TargetProfile.createRoute(friend.friendId)
                                                    )
                                                },
                                ...
                                Box(
                                    ...
                                    Column(horizontalAlignment = Alignment.CenterHorizontally) {
                                        ...
                                        Button(
                                            onClick = {
                                                currentUserId.let {
                                                    userFriendsViewModel.getFriends(it)
                                                }
                                            },
                                            colors = ButtonDefaults.buttonColors(
                                                containerColor = mainAppColor
    ...
    if (showUnfriendDialog && selectedFriend != null) {
        DeleteConfirmationDialog(
            title = "Remove Friend",
            itemName = selectedFriend!!.displayName,
            onDismissRequest = { showUnfriendDialog = false },
            onConfirm = {
                currentUserId.let { userId ->
                    userFriendsViewModel.deleteFriend(userId, selectedFriend!!.friendId)
                }
    ...
    if (showAddSuccessDialog) {
        ConfirmationDialog(
            ...
            onConfirm = { showAddSuccessDialog = false }
        )
    }
}